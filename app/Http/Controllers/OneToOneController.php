...
class OneToOneController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        // Redirect alla lista: il modale si chiude, il toast mostra il messaggio di conferma.
    ...
    public function updateStatus(Request $request, OneToOneRequest $oneToOneRequest): RedirectResponse
    {
        ...
        if ($request->boolean('confirm_completed')) {
            ...
            if ($oneToOneRequest->isFullyConfirmed()) {
                ...
                // Notifica l'altra parte: il primo che aveva confermato
                // non saprebbe altrimenti che l'incontro è stato chiuso.
                $otherUserId = $isRequester ? $oneToOneRequest->recipient_id : $oneToOneRequest->requester_id;
                $otherUser   = User::find($otherUserId);
                $otherUser?->notify(new OneToOneStatusChangedNotification($oneToOneRequest, OneToOneStatus::Completed, $user->name));
            } else {
