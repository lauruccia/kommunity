<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <div class="km-chip mb-3">Kommunity</div>
                <h1 class="text-3xl font-semibold text-stone-950">Piani di abbonamento</h1>
                <p class="mt-2 text-stone-500">Scegli il piano più adatto alla tua presenza nella community.</p>
            </div>
        </div>
    </x-slot>

    <div class="km-shell py-10">
        ...
        @if ($currentSubscription)
            <div class="mb-8 rounded-[1.6rem] border p-6
                ...
                @else border-stone-200 bg-stone-50
                @endif">
                <div class="flex flex-wrap items-center justify-between gap-4">
                    <div>
                        <div class="flex items-center gap-2">
                            ...
                            <span class="text-sm text-stone-600">{{ $currentSubscription->plan->name }}</span>
                        </div>
                    ...
                    @if($currentSubscription->status->value === 'pending')
                        <form method="POST" action="{{ route('subscriptions.cancel', $currentSubscription) }}"
                            ...
                            @csrf @method('DELETE')
                            <button type="submit" class="rounded-xl border border-rose-200 bg-white px-4 py-2 text-xs font-medium text-rose-600 transition hover:bg-rose-50">
                                Annulla richiesta
                            </button>
                        </form>
                    @endif
        ...
        {{-- Piani disponibili --}}
        @if ($plans->isEmpty())
            <div class="rounded-[1.6rem] border border-dashed border-stone-300 bg-stone-50 py-14 text-center text-stone-500">
        ...
        @else
            <div x-data="{ billing: 'monthly' }" class="space-y-8">
                ...
                <div class="flex justify-center">
                    <div class="inline-flex items-center rounded-full border border-stone-200 bg-white p-1 shadow-sm">
                        <button type="button"
                                @click="billing = 'monthly'"
                                :class="billing === 'monthly' ? 'bg-[color:var(--km-accent)] text-white shadow' : 'text-stone-500 hover:text-stone-700'"
                                class="rounded-full px-5 py-2 text-sm font-semibold transition">
                            Mensile
                        </button>
                        <button type="button"
                                @click="billing = 'yearly'"
                                :class="billing === 'yearly' ? 'bg-[color:var(--km-accent)] text-white shadow' : 'text-stone-500 hover:text-stone-700'"
                                class="rounded-full px-5 py-2 text-sm font-semibold transition">
                            Annuale
                ...
                <div class="grid gap-6 md:grid-cols-{{ min($plans->count(), 3) }}">
                    @foreach ($plans as $plan)
                        ...
                        @endphp
                        <div class="km-panel flex flex-col p-7 @if($plan->includesPage()) ring-2 ring-[color:var(--km-accent)] ring-offset-2 @endif">
                            ...
                            <h2 class="text-xl font-semibold text-stone-950">{{ $plan->name }}</h2>
                            ...
                            <div class="mt-5">
                                <div x-show="billing === 'monthly'">
                                    @if((float)$plan->price_monthly === 0.0)
                                        <span class="text-4xl font-black text-stone-950">Gratuito</span>
                                    @else
                                        <span class="text-4xl font-black text-stone-950">€{{ number_format((float)$plan->price_monthly, 0, ',', '.') }}</span>
                                        <span class="text-stone-400">/mese</span>
                                    @endif
                                </div>
                                <div x-show="billing === 'yearly'" x-cloak>
                                    @if((float)$plan->price_yearly === 0.0)
                                        <span class="text-4xl font-black text-stone-950">Gratuito</span>
                                    @else
                                        <span class="text-4xl font-black text-stone-950">€{{ number_format((float)$plan->price_yearly, 0, ',', '.') }}</span>
                                        <span class="text-stone-400">/anno</span>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="mt-7">
                                ...
                                @else
                                    <div class="rounded-2xl border border-stone-200 bg-stone-50 px-4 py-3 text-center text-sm text-stone-400">
                ...
                <div class="rounded-2xl border border-stone-200 bg-stone-50 px-6 py-5">
                    <p class="text-sm font-medium text-stone-700 mb-3">Metodi di pagamento accettati</p>
                    <div class="flex flex-wrap gap-4">
                        <div class="flex items-center gap-2 text-sm text-stone-600">
                            <svg class="h-5 w-5 text-stone-400" viewBox="0 0 20 20" fill="currentColor"><path d="M3 4a2 2 0 00-2 2v1h18V6a2 2 0 00-2-2H3zm16 5H1v5a2 2 0 002 2h14a2 2 0 002-2V9zM4 13a1 1 0 011-1h1a1 1 0 010 2H5a1 1 0 01-1-1zm5-1a1 1 0 000 2h1a1 1 0 100-2H9z"/></svg>
                            Carta di credito/debito
                        </div>
                        <div class="flex items-center gap-2 text-sm text-stone-600">
                            <svg class="h-5 w-5 text-stone-400" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M4 4a2 2 0 012-2h8a2 2 0 012 2v12a2 2 0 01-2 2H6a2 2 0 01-2-2V4zm3 1h6v4H7V5zm8 8v2h1v1H4v-1h1v-2a1 1 0 011-1h8a1 1 0 011 1z" clip-rule="evenodd"/></svg>
                            Bonifico bancario
                        </div>
                        <div class="flex items-center gap-2 text-sm text-stone-600">
                    ...
                    <p class="mt-3 text-xs text-stone-400">Dopo aver inviato la richiesta, effettua il pagamento e inserisci il riferimento nel form. L'abbonamento verrà attivato dopo la verifica da parte dell'amministratore.</p>
                </div>
            </div>
        @endif
    </div>
    ...
    <div x-data="{ open: false, plan_id: null, plan_name: '' }"
        ...
        <div class="relative w-full max-w-md rounded-[1.6rem] border border-stone-200 bg-white p-7 shadow-2xl"
            ...
            <h2 class="text-lg font-semibold text-stone-950 mb-1" x-text="'Richiedi: ' + plan_name"></h2>
            <p class="text-sm text-stone-500 mb-5">Scegli il metodo di pagamento e inserisci i dettagli. L'admin attiverà l'abbonamento dopo la verifica.</p>
            ...
            <form method="POST" action="{{ route('subscriptions.request') }}" class="space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-stone-700 mb-1">Metodo di pagamento</label>
                    <select name="payment_method" class="km-input" required>
                ...
                <div>
                    <label class="block text-sm font-medium text-stone-700 mb-1">Riferimento pagamento <span class="text-stone-400 font-normal">(opzionale)</span></label>
                    <input type="text" name="payment_reference" class="km-input"
                           placeholder="es. CRO bonifico, ID transazione PayPal…">
                    <p class="mt-1 text-xs text-stone-400">Puoi aggiungerlo anche dopo se non ce l'hai ancora.</p>
                    @error('payment_reference') <p class="mt-1 text-xs text-rose-600">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-medium text-stone-700 mb-1">Note aggiuntive <span class="text-stone-400 font-normal">(opzionale)</span></label>
                    <textarea name="payment_notes" rows="2" class="km-input"
