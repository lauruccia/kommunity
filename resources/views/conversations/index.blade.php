    ...
    <main class="km-shell-wide py-3">
        <section class="km-chat-layout">
                ...
                <div class="border-b border-white/[.08] p-4">
                    ...
                    <form method="GET" action="{{ route('conversations.index') }}" class="mt-4 flex gap-2">
                        <label class="relative min-w-0 flex-1">
                            <input name="search" value="{{ $filters['search'] ?? '' }}" class="km-dark-input h-11 rounded-xl pr-10" placeholder="Cerca un messaggio...">
                    ...
                    <div class="mt-4 flex items-center gap-2 text-xs font-black">
                        <a href="{{ route('conversations.index') }}" class="{{ $activeFilter === 'all' ? 'km-cta-primary' : 'km-cta-secondary' }} px-3 py-2">Tutti</a>
                        <a href="{{ route('conversations.index', ['filter' => 'unread']) }}" class="{{ $activeFilter === 'unread' ? 'km-cta-primary' : 'km-cta-secondary' }} px-3 py-2">Non letti</a>
                    </div>
                </div>
                ...
                <div class="min-h-0 flex-1 overflow-y-auto">
                    @forelse ($conversations as $conversation)
                        @php
                            ...
                            $isOnline = (bool) ($participant?->show_online_status ?? true)
                    ...
                    @empty
                        <div class="p-5 text-sm text-white/55">Nessun messaggio.</div>
                    @endforelse
                </div>
            </aside>
            ...
            <section class="km-chat-panel km-chat-main flex min-h-0 flex-col items-center justify-center p-8 text-center">
                ...
                <h1 class="mt-5 text-2xl font-black tracking-tight text-white">Seleziona un messaggio</h1>
                <p class="mt-2 max-w-md text-sm leading-6 text-white/55">Apri un messaggio dalla lista o inizia una nuova chat.</p>
            ...
            <aside class="km-chat-panel km-chat-detail p-5">
                ...
                <h2 class="mt-1 text-lg font-black text-white">Messaggi</h2>
                <div class="mt-5 rounded-2xl border border-white/[.08] bg-white/[.035] p-4 text-sm text-white/60">
                    <div class="flex items-center justify-between"><span>Messaggi</span><strong class="text-white">{{ $conversations->count() }}</strong></div>
                    <div class="mt-3 flex items-center justify-between"><span>Non letti</span><strong class="text-white">{{ $unreadCount }}</strong></div>
                </div>
                <p class="mt-4 text-sm leading-6 text-white/50">Seleziona un messaggio per visualizzare profilo, contatti, media e opzioni.</p>
            </aside>
        </section>
    </main>
        ...
        <div class="km-chat-panel w-full max-w-2xl">
            <div class="flex items-center justify-between border-b border-white/[.10] px-5 py-4">
                <div>
                    ...
                    <h2 class="mt-1 text-xl font-black text-white">Nuovo messaggio</h2>
                </div>
            ...
            <form method="POST" action="{{ route('conversations.start') }}" class="space-y-4 px-5 py-5">
                ...
                <button type="submit" class="km-button-primary w-full">Invia messaggio</button>
            </form>
        </div>
    </div>
