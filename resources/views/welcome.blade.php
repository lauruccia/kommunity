...
<head>
    ...
    <style>
        ...
        *{box-sizing:border-box}
        ...
        .map-area-pin{position:absolute;z-index:4;left:var(--x);top:var(--y);display:inline-flex;align-items:center;gap:.38rem;transform:translate(-.65rem,-50%);font-size:.68rem;font-weight:900;color:#dff5d8;text-shadow:0 2px 10px rgba(0,0,0,.75);white-space:nowrap}.map-area-pin::before{content:"";width:.78rem;height:.78rem;border-radius:999px;background:var(--brand4);box-shadow:0 0 0 5px rgba(143,207,125,.14),0 0 22px rgba(143,207,125,.92)}.map-area-panel{position:absolute;right:0;bottom:3.2rem;z-index:5;width:min(18rem,42%);border-radius:.86rem;padding:1rem}.map-area-panel-title{font-size:.78rem;font-weight:900;color:rgba(214,228,236,.78);margin:0 0 .8rem}.map-area-list{display:flex;flex-wrap:wrap;gap:.5rem;margin:0;padding:0;list-style:none}.map-area-list li{display:inline-flex;align-items:center;gap:.38rem;border:1px solid rgba(111,163,103,.32);border-radius:.45rem;background:rgba(7,17,26,.45);padding:.42rem .58rem;font-size:.72rem;font-weight:900;color:rgba(240,246,250,.88)}.map-area-list li::before{content:"";width:.44rem;height:.44rem;border-radius:999px;background:var(--brand4);box-shadow:0 0 11px rgba(143,207,125,.82)}
...
@php
    ...
    $activeAreaNames = $chapters
        ->map(fn ($ch) => $ch->city?->name ?? $ch->name)
        ->filter()
        ->unique()
        ->take(8)
        ->values();
    if ($activeAreaNames->isEmpty()) {
...
<section id="community-locale" class="local-section km-section">
    <div class="km-wrap local-grid">
        <div>
            ...
            <p class="section-copy">Kommunity cresce con Pianeti professionali attivi in più territori. Trova il contesto più vicino a te e inizia a connetterti.</p>
            <p style="margin-top:1.35rem;font-size:.8rem;font-weight:900;color:rgba(214,228,236,.78)">Aree attive</p>
            <div class="city-chips">
                @foreach($activeAreaNames as $area)<span class="city-chip">{{ $area }}</span>@endforeach
        ...
        <div class="map-wrap">
            <div class="italy-map-art" aria-label="Aree attive Kommunity in Italia">
                ...
                @endforeach
                <div class="map-area-panel glass">
                    <p class="map-area-panel-title">Pianeti attivi</p>
                    <ul class="map-area-list">
                        @foreach($activeAreaNames->take(6) as $area)<li>{{ $area }}</li>@endforeach
                    </ul>
                    <span style="display:inline-flex;margin-top:.8rem;font-size:.78rem;font-weight:900;color:var(--brand4)">Accesso su invito</span>
                </div>
            </div>
        </div>
    </div>
