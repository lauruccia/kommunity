<x-app-layout>
    ...
    <div class="pb-12">
        <div class="w-full px-4 pt-8 sm:px-6 lg:px-8">
            <div class="mb-6 w-full rounded-[2rem] p-6 text-white shadow-[0_22px_60px_rgba(66,87,103,0.22)]" style="background: linear-gradient(135deg, #425767 0%, #4d6474 52%, #5b7d4b 100%);">
                <p class="text-xs uppercase tracking-[0.24em]" style="color: rgba(255,255,255,0.70);">Networking tra membri</p>
                <div class="mt-2 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    <h1 class="font-serif text-2xl font-semibold sm:text-3xl">Richieste one-to-one</h1>
                    <button type="button" id="open-one-to-one-create-modal" class="km-button-primary shrink-0">+ Nuova richiesta</button>
                </div>
                <p class="mt-3 text-sm leading-7" style="color: rgba(255,255,255,0.80);">Se l'orario richiesto rientra in una disponibilita' attiva del destinatario e non ci sono conflitti, il one-to-one viene prenotato subito. Altrimenti parte come richiesta da confermare.</p>
                <div class="mt-4 flex flex-wrap gap-3">
                    <div class="flex items-center gap-2 rounded-full px-4 py-2 text-sm border" style="background: rgba(255,255,255,0.15); border-color: rgba(255,255,255,0.25);">
                        <span class="font-bold text-white">{{ $summary['total'] }}</span>
                        <span style="color: rgba(255,255,255,0.75);">totali</span>
                    </div>
                    <div class="flex items-center gap-2 rounded-full px-4 py-2 text-sm border" style="background: rgba(255,255,255,0.15); border-color: rgba(255,255,255,0.25);">
                        <span class="font-bold text-white">{{ $summary['received'] }}</span>
                        <span style="color: rgba(255,255,255,0.75);">ricevuti</span>
                    </div>
                    <div class="flex items-center gap-2 rounded-full px-4 py-2 text-sm border" style="background: rgba(255,255,255,0.15); border-color: rgba(255,255,255,0.25);">
                        <span class="font-bold text-white">{{ $summary['sent'] }}</span>
                        <span style="color: rgba(255,255,255,0.75);">inviati</span>
                    </div>
                </div>
            </div>
            <div class="flex flex-col gap-6 lg:items-start" style="flex-direction: row; align-items: flex-start;">
            <aside class="space-y-6 lg:sticky lg:top-6" style="width: 340px; min-width: 340px; flex-shrink: 0;">
            ...
            <section class="space-y-5" style="flex: 1; min-width: 0;">
                        ...
                        <form method="POST" action="{{ route('one-to-ones.status', $selectedRequest) }}" class="grid gap-3 rounded-[1.6rem] border border-stone-200 bg-stone-50 p-4 xl:w-[320px]">
                            ...
                            @if ($isRequester)
                                ...
                                @if (!in_array($selectedRequest->status->value, ['completed', 'cancelled']))
                                    <div class="rounded-[1.1rem] border border-rose-200 bg-rose-50 p-3">
                                        <p class="text-xs uppercase tracking-[0.18em] text-rose-700">Annulla richiesta</p>
                                        <p class="mt-1 text-xs leading-5 text-rose-600">Puoi annullare questa richiesta finche' non e' completata. L'altra parte sara' avvisata dal cambio di stato.</p>
                                        <button type="submit" name="status" value="cancelled" class="mt-2 rounded-full border border-rose-300 bg-white px-4 py-2 text-xs font-semibold text-rose-700 transition hover:bg-rose-100" onclick="return confirm('Sei sicuro di voler annullare questa richiesta?')">
                                            Annulla richiesta
                                        </button>
                                    </div>
                                @endif
