...
<div
    ...
    x-data="{
        ...
        defaultEventId: {{ \Illuminate\Support\Js::from($defaultEventId) }},
        ...
        isEventVisible(id) {
            ...
            if (!ev) return false;
            if (this.searchQuery && !ev.title.toLowerCase().includes(this.searchQuery.toLowerCase())) return false;
...
<div class="flex flex-none flex-wrap items-center gap-3 px-4 py-5 lg:flex-nowrap lg:px-8">
    ...
    <button
        ...
        title="Nuovo evento"
    >
        <svg class="h-5 w-5 flex-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M12 5v14M5 12h14"/></svg>
    </button>
    @endif
    ...
    <div class="flex items-center gap-3">
        <a href="{{ route('events.index', ['view' => $viewMode, 'month' => now()->format('Y-m'), 'day' => now()->format('Y-m-d')]) }}"
           class="hidden rounded-full border border-white/15 px-5 py-3 text-sm font-bold text-white/78 transition hover:bg-white/[.07] hover:text-white sm:block">Oggi</a>
        ...
        <div class="min-w-[140px] text-center sm:min-w-[190px]">
            <div class="text-xl font-bold text-white">
                @if ($viewMode === 'month'){{ $monthDate->translatedFormat('F Y') }}
                @elseif ($viewMode === 'week'){{ $weekStart->translatedFormat('d M') }} - {{ $weekEnd->translatedFormat('d M Y') }}
                @elseif ($viewMode === 'day'){{ $selectedDay->translatedFormat('d F Y') }}
                @else{{ $monthDate->translatedFormat('F Y') }}
                @endif
            </div>
        </div>
    ...
    <div class="relative ml-auto w-full sm:w-[260px] xl:w-[320px]">
        <svg class="pointer-events-none absolute right-4 top-1/2 h-5 w-5 -translate-y-1/2 text-white/56" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
        <input
            type="text"
    ...
    <button
        ...
        title="Filtri"
    >
        <svg class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4 6h16M7 12h10M10 18h4"/></svg>
        Filtri
    </button>
</div>
...
{{-- ════════════════════════════════════════════════════════════════
     CORPO: SIDEBAR + CALENDARIO + PANNELLO DETTAGLIO
════════════════════════════════════════════════════════════════ --}}
<div class="km-events-layout grid flex-1 gap-4 overflow-hidden px-4 pb-6 lg:px-8" style="min-height: calc(100vh - 10rem);">
    ...
    >
        ...
        <div class="mb-5">
            ...
            <div class="mb-1.5 grid grid-cols-7 text-center text-[10px] uppercase tracking-[0.14em] text-white/40">
                @foreach ($miniWeekdayLabels as $ml)<div>{{ $ml }}</div>@endforeach
            </div>
            ...
            <div class="mb-0.5 grid grid-cols-7">
                ...
                >
                    {{ $day['date']->format('j') }}
                    @if ($day['events']->isNotEmpty() && ! $miniSel)
        ...
        <div class="mb-5">
            ...
            <div class="space-y-2">
                ...
                >
                    <span class="flex items-center gap-3"><span class="km-events-filter-dot {{ $filterDef['dot'] }}"></span>{{ $filterDef['label'] }}</span>
                    <span
        ...
        <div class="mt-5 border-t border-white/[.07] pt-4">
            ...
            @php $st = $eventStatuses[$ev->id] ?? null; @endphp
            <button
            ...
            >
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-[10px] uppercase tracking-[0.14em] text-white/45">{{ $ev->starts_at->format('H:i') }}</span>
                </div>
                <div class="mt-1 truncate text-xs font-semibold text-white">{{ $ev->title }}</div>
                <div class="mt-0.5 truncate text-[11px] text-white/50">{{ $ev->location ?: 'Online' }}</div>
            </button>
            @empty
    ...
    <main class="km-events-card min-w-0 overflow-auto rounded-lg">
        ...
        <div class="sticky top-0 z-10 grid grid-cols-7 border-b border-white/10 bg-[rgba(4,28,40,0.96)]">
            ...
            <div class="border-r border-white/[.06] px-2 py-5 text-center text-[12px] uppercase tracking-[0.14em] text-white/48 last:border-r-0">{{ mb_strtoupper(mb_substr($label, 0, 3)) }}</div>
            @endforeach
        </div>
        ...
        <div class="grid grid-cols-7">
            ...
            @endphp
            <div class="km-events-day p-3 last:border-r-0
                {{ $isSelected ? 'bg-[rgba(85,121,79,0.08)]' : ($isCurrentMonth ? 'bg-white/[.02]' : '') }}">
                <div class="mb-1 flex items-center justify-between px-0.5">
                    ...
                    <span class="text-[10px] text-white/30">{{ $day['events']->count() }}</span>
                    @endif
                </div>
                    ...
                    >
                        <div class="flex items-start justify-between gap-2">
                            <div class="min-w-0">
                                ...
                                <div class="line-clamp-2 font-semibold">{{ $ev->title }}</div>
                            </div>
                            @if ($ev->attending_count > 0)
        ...
        <div class="sticky top-0 z-10 grid grid-cols-[56px_repeat(7,minmax(0,1fr))] border-b border-white/10 bg-[rgba(22,30,40,0.97)]">
            ...
            @php $dSel = $day['date']->isSameDay($selectedDay); $dToday = $day['date']->isToday(); @endphp
            <a
            ...
            >
                <div class="text-[10px] uppercase tracking-[0.14em] text-white/40">{{ $weekdayLabels[$idx] }}</div>
                <div class="mx-auto mt-1 inline-flex h-8 w-8 items-center justify-center rounded-full text-base font-semibold
                    {{ $dToday ? 'bg-[color:var(--km-events-green)] text-white' : ($dSel ? 'bg-white/15 text-white' : 'text-white/75') }}">
                    {{ $day['date']->format('d') }}
                </div>
                @if ($day['events']->count() > 0)
                <div class="mt-0.5 text-[10px] text-white/35">{{ $day['events']->count() }} ev.</div>
                @endif
            </a>
            @endforeach
        </div>
        ...
        @php
            ...
            foreach ($weekDays as $wIdx => $wDay) {
                $slotByDay[$wIdx] = $wDay['events']->filter(fn($e) => (int)$e->starts_at->format('G') === $hour)->values();
                if ($slotByDay[$wIdx]->isNotEmpty()) $hasAnySlot = true;
            }
        ...
        <div class="grid grid-cols-[56px_repeat(7,minmax(0,1fr))] border-b border-white/[.05] {{ $hour < 8 ? 'opacity-50' : '' }}">
            ...
            <div class="min-h-[52px] border-r border-white/[.05] p-1 last:border-r-0 {{ $wDay['date']->isSameDay($selectedDay) ? 'bg-[rgba(85,121,79,0.04)]' : '' }}">
                ...
                @php
                    $s    = $eventStatuses[$ev->id] ?? null;
                    $pill = $ev->status === 'cancelled' ? 'bg-red-500/15 text-red-300 border-red-500/25' : ($ev->starts_at->isPast() ? 'bg-white/[.06] text-white/55 border-white/[.10]' : match($s) {
        ...
        @foreach (range(0, 23) as $hour)
        @php
            $slotEvs = $selectedDayEvents->filter(fn($e) => (int)$e->starts_at->format('G') === $hour);
                ...
                <div class="space-y-2">
                    ...
                    >
                        <div class="text-[11px] uppercase tracking-[0.14em] opacity-65">
                            {{ $ev->starts_at->format('H:i') }}@if($ev->ends_at) - {{ $ev->ends_at->format('H:i') }}@endif
                        </div>
                        <div class="mt-1 font-semibold">{{ $ev->title }}</div>
        ...
        <div class="space-y-3 p-4">
            ...
            @php
                $s = $eventStatuses[$ev->id] ?? null;
                $listPill = $ev->status === 'cancelled' ? 'border-red-400/20 bg-red-500/10' : ($s === 'attending' || $s === 'registered' ? 'border-[color:var(--km-events-green)]/30 bg-[color:var(--km-events-green)]/12' : 'border-white/10 bg-white/[.035]');
            @endphp
            <button
                x-show="isEventVisible({{ $ev->id }})"
                @click="showEvent({{ $ev->id }})"
            ...
            >
                ...
                <div class="min-w-0 flex-1">
                    <div class="truncate text-base font-bold text-white">{{ $ev->title }}</div>
                    <div class="mt-1 text-sm text-white/55">{{ $ev->starts_at->format('H:i') }}@if($ev->ends_at) - {{ $ev->ends_at->format('H:i') }}@endif &middot; {{ $ev->location ?: 'Online' }}</div>
                </div>
                <span class="hidden rounded-full border border-white/10 px-3 py-1 text-xs font-bold text-white/55 sm:inline">{{ $ev->attending_count }} partecipanti</span>
            </button>
            @empty
            <div class="rounded-md border border-dashed border-white/10 p-10 text-center text-white/45">Nessun evento nel periodo selezionato</div>
            @endforelse
        </div>
        @endif
    ...
    >
        <div class="flex h-full w-full flex-col overflow-y-auto">
            ...
            {{-- Placeholder quando nessun evento selezionato --}}
            <div
                x-show="!detailOpen"
            ...
            {{-- Contenuto visibile solo quando evento selezionato --}}
            ...
            <div x-show="selectedEvent && selectedEvent.cover_image" class="h-44 w-full flex-none overflow-hidden">
                <img :src="selectedEvent?.cover_image" :alt="selectedEvent?.title" class="h-full w-full object-cover">
            </div>
            ...
            <div class="flex flex-1 flex-col gap-5 p-5">
                ...
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    ></span>
                    <span class="ml-auto inline-flex items-center rounded-full border border-white/10 bg-white/[.06] px-3 py-1.5 text-[11px] font-bold text-white/55" x-text="selectedEvent?.type_label"></span>
                ...
                <div>
                    ...
                    <h2 class="text-2xl font-bold leading-tight text-white" x-text="selectedEvent?.title"></h2>
                    <p class="mt-2 text-sm text-white/58" x-text="selectedEvent?.location"></p>
                </div>

                {{-- Data / ora --}}
                <div class="flex items-start gap-3 rounded-2xl border border-white/[.07] bg-white/[.03] px-4 py-3">
                    <svg class="mt-0.5 h-4 w-4 flex-none text-[color:var(--km-events-green)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>
                    <div>
                        <div class="text-sm font-semibold text-white" x-text="selectedEvent?.date_label"></div>
                        <div class="text-xs text-white/50" x-text="selectedEvent?.time_label"></div>
                    </div>
                </div>

                {{-- Luogo / link meeting --}}
                <div class="flex items-start gap-3 rounded-2xl border border-white/[.07] bg-white/[.03] px-4 py-3">
                    <svg class="mt-0.5 h-4 w-4 flex-none text-[color:var(--km-events-green)]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M12 21c-4-4-7-7.5-7-10.5a7 7 0 0 1 14 0c0 3-3 6.5-7 10.5Z"/><circle cx="12" cy="10" r="2.5"/></svg>
                    <div class="min-w-0">
                        <div class="text-sm font-semibold text-white" x-text="selectedEvent?.location"></div>
                        <a
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div class="text-sm font-semibold text-white" x-text="selectedEvent?.organizer_name"></div>
                    </div>
                </div>
                ...
                <div x-show="selectedEvent?.attendees_preview && selectedEvent.attendees_preview.length > 0">
                    ...
                    <div class="flex flex-wrap gap-1.5">
                        <template x-for="att in (selectedEvent?.attendees_preview ?? [])" :key="att.name">
                            <div
                        ...
                        <div
                            ...
                            x-text="'+' + (selectedEvent.attending_count - 8)"
                        ></div>
                    </div>
                </div>

                {{-- Azioni RSVP (non manager, evento non passato/annullato) --}}
                ...
                <div x-show="selectedEvent?.can_manage" class="space-y-2">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        ...
                        >
                            ...
                            @method('PATCH')
                            <button type="submit"
                                class="flex w-full items-center justify-center gap-1.5 rounded-xl border border-red-400/20 bg-red-500/10 px-3 py-2.5 text-xs font-semibold text-red-300 transition hover:bg-red-500/20"
                            >
                                <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 6 6 18M6 6l12 12"/></svg>
                                Annulla
                            </button>
                    ...
                    <a :href="selectedEvent?.detail_url" class="flex w-full items-center justify-center gap-1.5 rounded-xl border border-white/10 bg-white/[.04] px-4 py-2.5 text-xs font-semibold text-white/65 transition hover:bg-white/[.07]">
                        <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75"/></svg>
                        Gestisci inviti
                    </a>
                </div>

                {{-- Link dettaglio completo --}}
                <a :href="selectedEvent?.detail_url"
            ...
            </div>{{-- fine wrapper x-show="detailOpen" --}}
        </div>
    </aside>

</div>{{-- fine tre pannelli --}}


{{-- ════════════════════════════════════════════════════════════════
     MODAL CREA EVENTO (solo manager)
════════════════════════════════════════════════════════════════ --}}
@if ($canManageEvents)
<div
    x-cloak
        ...
        >
            ...
            <form method="POST" action="{{ route('events.store') }}" enctype="multipart/form-data" class="divide-y divide-white/[.07]">
                    ...
                    <div x-show="inviteTarget === 'users'" x-cloak class="space-y-2">
                        ...
                        <div class="max-h-48 overflow-y-auto rounded-2xl border border-white/10 bg-white/[.02]">
                            <template x-for="u in filteredUsers" :key="u.id">
                                ...
                                >
                                    <input type="checkbox" name="invite_user_ids[]" :value="u.id" @change="toggleUser(u.id)" :checked="selectedUserIds.includes(u.id)" class="rounded border-white/30 bg-white/10 text-[color:var(--km-events-green)] focus:ring-[color:var(--km-events-green)]/30">
                ...
                <div class="flex items-center justify-end gap-3 px-6 py-4">
                    <button type="button" @click="createOpen = false" class="km-button-secondary px-5 py-2.5 text-sm">Annulla</button>
