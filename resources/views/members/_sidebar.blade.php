...
@php
    ...
    $sidebarAvgRating  = $reviewsWithRating->isNotEmpty()
        ...
        : null;
    $sidebarReviews    = $reviews->take(5);
    $totalReviews      = $reviews->count();
@endphp
<div class="km-panel p-6">

    {{-- Intestazione --}}
    <h3 class="text-sm font-semibold uppercase tracking-[0.18em] text-stone-500">Recensioni da Kommunity</h3>

    {{-- Media stelle --}}
    @if ($sidebarAvgRating)
    <div class="mt-3 flex items-center gap-3">
        <span class="text-3xl font-bold text-stone-900 leading-none">{{ number_format($sidebarAvgRating, 1) }}</span>
        <div>
            <span class="flex gap-0.5">
                @for ($i = 1; $i <= 5; $i++)
                    <svg class="h-4 w-4 {{ $i <= floor($sidebarAvgRating) ? 'text-yellow-400' : 'text-stone-200' }}" viewBox="0 0 20 20" fill="currentColor">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                    </svg>
                @endfor
            </span>
            <p class="mt-0.5 text-xs text-stone-400">su {{ $totalReviews }} {{ $totalReviews === 1 ? 'recensione' : 'recensioni' }}</p>
        </div>
    </div>
    @else
    <p class="mt-2 text-sm text-stone-400">{{ $totalReviews }} {{ $totalReviews === 1 ? 'recensione' : 'recensioni' }}</p>
    @endif
    ...
    <div class="mt-4 space-y-3">
        ...
        <div class="rounded-xl border border-stone-100 bg-stone-50 p-3">
            ...
            <div class="flex items-center gap-2">
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <span class="mt-0.5 flex gap-0.5">
                        @for ($i = 1; $i <= 5; $i++)
                            <svg class="h-3 w-3 {{ $i <= $review->rating ? 'text-yellow-400' : 'text-stone-200' }}" viewBox="0 0 20 20" fill="currentColor">
                                <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                            </svg>
                        @endfor
                    </span>
            ...
            {{-- Testo --}}
            @if ($review->content)
                <p class="mt-2 line-clamp-3 text-xs leading-relaxed text-stone-600">{{ $review->content }}</p>
            @endif
        </div>
        @endforeach
    </div>
    ...
    <a href="{{ route('members.reviews', $onepage->slug) }}"
